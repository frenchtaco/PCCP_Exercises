# Code Lecture | Week 05

The structure of this week's Gradle project is a bit special because we use external libraries and plugins for [JUnit](https://junit.org/) and [JCStress](https://github.com/openjdk/jcstress/tree/master). This file contains a few clarifications regarding the content of this directory.
...
* JUnit tests are located in the directory `src/test/java/lecture05/`. This is an important detail, as JUnit tests are the ones we will use throughout the course.

* To run JUnit tests with gradle, execute `$ gradle cleanTest test --tests <package>.<test_class>` from the root of the project. For instance, `$ gradle cleanTest test --tests lecture05.CounterTest`.
...
* The example JCStress test is located in the directory `src/jcstress/java/lecture05/`.
